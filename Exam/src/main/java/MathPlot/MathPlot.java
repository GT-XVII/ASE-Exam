...
public class MathPlot {
    ...
    public void setExpression(String expr, ExpressionFormat format) {
        // YOU CAN CHANGE HERE
        ExpressionEngine.setExpression(expr, format);
    }
    ...
    public void plot(Canvas canvas, PlotType type) {
        ...
        // Plot function and derivative (if any)
        if (ExpressionEngine.hasFunction()) {
            double step = 0.05;

            if (type == PlotType.Cartesian) {
                Point.Iterator fIt = ExpressionEngine.buildCartesianIterator(false, -10.0, 10.0, step);
                pf.addCurve(fIt, Color.BLUE, 1.5);

                if (ExpressionEngine.hasDerivative()) {
                    Point.Iterator dfIt = ExpressionEngine.buildCartesianIterator(true, -10.0, 10.0, step);
                    pf.addCurve(dfIt, Color.RED, 1.5);
                }
            } else { // Polar
                double thetaStart = 0.0;
                double thetaEnd = 2.0 * Math.PI;

                Point.Iterator fIt = ExpressionEngine.buildPolarIterator(false, thetaStart, thetaEnd, step);
                pf.addCurve(fIt, Color.BLUE, 1.5);

                if (ExpressionEngine.hasDerivative()) {
                    Point.Iterator dfIt = ExpressionEngine.buildPolarIterator(true, thetaStart, thetaEnd, step);
                    pf.addCurve(dfIt, Color.RED, 1.5);
                }
            }
        }
    ...
    public double area(AreaType areaType) {
        // YOU CAN CHANGE HERE
        if (!ExpressionEngine.hasFunction() || areaType == null) {
            return Double.NaN;
        }

        return switch (areaType) {
            case Rectangular -> ExpressionEngine.areaRectangular();
            case Trapezoidal -> ExpressionEngine.areaTrapezoidal();
        };
    }

    // This is to satisfy App.java
    public double area() {
    return area(AreaType.Rectangular);
}
    ...
    public List<String> print(ExpressionFormat format) {
        ...
        return ExpressionEngine.print(format);
    }
}